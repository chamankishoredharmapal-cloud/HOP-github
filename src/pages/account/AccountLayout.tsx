...
export default function AccountLayout() {
  ...
  return (
    <PageLayout>
       <div className="hop-account-shell">
        <h1 className="font-serif text-2xl md:text-3xl text-ink mb-8">My Account</h1>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          <nav className="hop-account-nav md:col-span-1" aria-label="Account navigation">
            {navItems.map((item) => (
              <NavLink
                key={item.to}
                to={item.to}
                ...
                className={({ isActive }) =>
                   `hop-account-nav-link ${isActive ? "is-active" : ""}`
                }
              >
            ...
            <button
              onClick={signOut}
               className="hop-account-nav-link w-full mt-4"
            >
